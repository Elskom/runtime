...
internal sealed partial class BlowFishInternal
{
    [DisposeField(false)]
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="BlowFishInternal"/> class with the hex key provided.
    /// </summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="BlowFishInternal"/> class with the byte key provided.
    /// </summary>
    ...
    internal bool NonStandard { get; set; }
    ...
    internal byte[] EncryptCBC(byte[] pt)
    {
    ...
    [Obsolete("Use CBC version.")]
    internal string EncryptECB(string pt)
        => ByteToHex(this.EncryptECB(Encoding.ASCII.GetBytes(pt)));

    [Obsolete("Use CBC version.")]
    internal byte[] EncryptECB(byte[] pt)
    {
        ArgumentNullException.ThrowIfNull(pt);
    ...
    [Obsolete("Use CBC version.")]
    internal string DecryptECB(string ct)
    {
        ArgumentNullException.ThrowIfNull(ct);
    ...
    [Obsolete("Use CBC version.")]
    internal byte[] DecryptECB(byte[] ct)
    {
        ArgumentNullException.ThrowIfNull(ct);
        return this.Decrypt_ECB(HexToByte(ct.AsSpan())).Remove((byte)'\0');
    }

    [Obsolete("Use CBC version.")]
    internal byte[] Decrypt_ECB(byte[] ct)
        => this.Crypt_ECB(ct, true);

    internal byte[] SetRandomIV()
    {
        Span<byte> iv = stackalloc byte[8];
        this.randomSource.GetBytes(iv);
    ...
    private static byte[] HexToByte(ReadOnlySpan<byte> hex)
    {
        Span<byte> r = stackalloc byte[hex.Length / 2];
        for (var i = 0; i < hex.Length - 1; i += 2)
        {
            var a = GetHex((char)hex[i]);
            var b = GetHex((char)hex[i + 1]);
            r[i / 2] = (byte)((a * 16) + b);
        }
    ...
    private void Encipher()
    {
        ...
        // swap the blocks
        (this.xlPar, this.xrPar) = (this.xrPar, this.xlPar);
    }

    private void Decipher()
    {
        this.xlPar ^= this.bfP[17];
        for (uint i = 16; i > 2; i -= 2)
        {
