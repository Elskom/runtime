...
public sealed partial class BlowFish
{
    ...
    {
        ArgumentNullException.ThrowIfNull(hexKey, nameof(hexKey));
    ...
    {
        ArgumentNullException.ThrowIfNull(cipherKey, nameof(cipherKey));
    ...
    public IEnumerable<byte> IV
    {
        ...
        set
        {
            ThrowInvalidOperation(value.ToArray().Length != 8, Resources.BlowFish_Invalid_IV_Size);
    ...
    {
        ArgumentNullException.ThrowIfNull(pt, nameof(pt));
    ...
    private static void ThrowInvalidOperation(bool value, string message)
    {
        if (value)
        {
    ...
    private void SetupKey(Span<byte> cipherKey)
    {
        ...
        ThrowInvalidOperation(cipherKey.Length > 56, Resources.BlowFish_Key_Too_Long);
        var j = 0;
        for (var i = 0; i < 18; i++)
        {
    ...
    {
        ThrowInvalidOperation(!this.iVSet, Resources.BlowFish_IV_Not_Set);
        var paddedLen = text.Length % 8 == 0 ? text.Length : text.Length + 8 - (text.Length % 8);
        if (paddedLen != text.Length)
        {
            Array.Resize(ref text, paddedLen);
