...
public static class KOMManager
{
    ...
    public static void UnpackKoms()
    {
        ...
        foreach (var fi in di.EnumerateFiles("*.kom").Select(fi => fi.Name))
        {
            var kom_file = fi;
    ...
    public static void PackKoms()
    {
        PackingState = true;
        DirectoryInfo di = new($"{Directory.GetCurrentDirectory()}{Path.DirectorySeparatorChar}koms");
        foreach (var dri in di.GetDirectories().Select(dri => dri.Name))
        {
            var kom_data_folder = dri;
    ...
    public static void ConvertCRC(int toVersion, string crcpath)
    {
        if (File.Exists(crcpath))
        {
            ...
            {
                foreach (var plugin in Komplugins.Where(
                    plugin => toVersion.Equals(plugin.SupportedKOMVersion)))
                {
    ...
    public static void UpdateCRC(int crcversion, string crcpath, string checkpath)
    {
        ...
        foreach (var fi1 in di1.EnumerateFiles().Select(fi1 => fi1.Name))
        {
            if (!fi1.Equals(crcfile.Name, StringComparison.Ordinal))
            {
                var found = false;
                ...
                if (crcversion > 2)
                {
                    foreach (var fileElement in xml.Elements("File"))
                    {
                        ...
                        if (name.Equals(fi1, StringComparison.Ordinal))
                        {
                            found = true;
                        }
                    }
                ...
                {
                    foreach (var plugin in Komplugins.Where(
                        plugin => crcversion.Equals(plugin.SupportedKOMVersion)))
                    {
    ...
    {
        var ret = 0;
        foreach (var supportedKomVersion in Komplugins.Select(komplugin => komplugin.SupportedKOMVersion))
        {
            if (supportedKomVersion != 0 && File.Exists($"{datafolder}{Path.DirectorySeparatorChar}KOMVERSION.{supportedKomVersion}"))
            {
                try
                {
                    File.Delete($"{datafolder}{Path.DirectorySeparatorChar}KOMVERSION.{supportedKomVersion}");
                    ret = supportedKomVersion;
                }
                catch (IOException)
                {
                    ret = -1;
